...
class v_1_0_2_ucp_module extends \phpbb\db\migration\migration
{
	...
	public function update_langname()
	{
		$this->db->sql_query(
			...
			SET module_langname = 'UCP_ACCOUNT'
			WHERE module_langname = 'UCP_TITLE'"
		);

		$this->db->sql_query(
			'UPDATE ' . MODULES_TABLE . "
			SET module_langname = 'UCP_ACCOUNT_ADD_KEY'
			WHERE module_langname = 'UCP_ADD_KEY'"
		);

		$this->db->sql_query(
			'UPDATE ' . MODULES_TABLE . "
			SET module_langname = 'UCP_ACCOUNT_MANAGE_KEYS'
			WHERE module_langname = 'UCP_MANAGE_KEYS'"
		);

		$this->db->sql_query(
			'UPDATE ' . MODULES_TABLE . "
			SET module_langname = 'UCP_ACCOUNT_SELECT_BADGES'
			WHERE module_langname = 'UCP_SELECT_BADGES'"
		);

		$this->db->sql_query(
			'UPDATE ' . MODULES_TABLE . "
			SET module_langname = 'UCP_ACCOUNT_ORDER_BADGES'
	...
	public function revert_data()
	{
		return array(
			...
			array('module.remove', array(
				'ucp',
				'UCP_ACCOUNT',
				'UCP_ACCOUNT_MANAGE_KEYS'
			)),
			array('module.remove', array(
				'ucp',
				'UCP_ACCOUNT',
				'UCP_ACCOUNT_SELECT_BADGES'
			)),
			array('module.remove', array(
				'ucp',
				'UCP_ACCOUNT',
				'UCP_ACCOUNT_ORDER_BADGES'
			)),
			array('module.remove', array(
				'ucp',
				0,

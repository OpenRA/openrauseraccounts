...
class core
{
	...
	/**
	 * Gets the url to the user avatar from the <img> tag generated by phpBB.
	 * Returns empty string on failure.
	 *
	 * @param array $user_data Avatar data from the user table
	 * @return string
	 */
	public function get_avatar_src($user_data) {
		$avatar_img = phpbb_get_avatar([
			'avatar' => $user_data['user_avatar'],
			'avatar_type' => $user_data['user_avatar_type'],
			'avatar_width' => $user_data['user_avatar_width'],
			'avatar_height' => $user_data['user_avatar_height'],
		], '');

		if (preg_match("/\<img.+src\=(?:\"|\')(.+?)(?:\"|\')(?:.+?)\>/", $avatar_img, $matches)) {
			return $matches[1];
		}
